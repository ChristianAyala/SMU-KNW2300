...
FOR /D /r %%i IN (bin*) ( COPY  %save%\libs\32-bit\rxtxSerial.dll %%i )
set javafound=y
goto checkRegular
...
FOR /D /r %%i IN (bin*) ( COPY %save%\libs\64-bit\rxtxSerial.dll %%i )
set javafound=y
goto closing
...
:closing
IF %javafound%==y ( ECHO "ERROR: No Java installation was found" ) ELSE ( ECHO "Java Installation has completed SUCCESSFULLY" )
goto end
